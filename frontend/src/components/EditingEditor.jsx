...
export default function EditingEditor({ processName, ruleName }) {
    ...
    const [processAcName, setProcessAcName] = useState({title: processName})

    const [selectedEntities, setSelectedEntities] = useState(() => {
        const keys = ['includeOT', 'includeAct', 'excludeOT', 'excludeAct'];
        const transformed = {};
            ...
            if (process.processName === processName){
                process.rules?.forEach(rule => {
                    if (rule.ruleName === ruleName){
                        keys.forEach(key => {
                            const { entities = [], condition = [] } = rule[key] || {};
                            transformed[key] = [...entities, ...condition];
                        });
                    }
                })
            }
    ...
    const [ruleData, setRuleData] = useState(() => {
        for (const process of processData) {
            if (process.processName === processName) {
            const foundRule = process.rules?.find(rule => rule.ruleName === ruleName);
            if (foundRule) {
                return foundRule;
            }
            }
        ...
        if (!allEmpty) {
            setProcessData(prev => {
                ...
                if (existingIndex !== -1) {
                    ...
                    const updatedRules = (() => {
                        const ruleIndex = existingProcess.rules.findIndex(
                            rule => rule.ruleName === updatedRuleData.ruleName
                        );

                        if (ruleIndex !== -1) {
                            const rulesCopy = [...existingProcess.rules];
                            rulesCopy[ruleIndex] = {
                                ...rulesCopy[ruleIndex],
                                ...updatedRuleData
                            };
                            ...
                            return [...existingProcess.rules, updatedRuleData];
                        }
                    })();
                    ...
                    updatedProcesses[existingIndex] = {
                        ...existingProcess,
                        rules: updatedRules,
                        relations: existingProcess.relations
                    };
                    ...
                    <form onSubmit={handleSubmit}>
                        <Box sx={{ m: 2, width: 380 }}>
                            ...
                            >
                                ...
                                <Box sx={{ width: '224px' }}>
                                    <RuleNameInput ruleName={ruleName} />
                                </Box>
                            </Stack>
                        </Box>
                        <Divider sx={{ m: 2 }}/>
