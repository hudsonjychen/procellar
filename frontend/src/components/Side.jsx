...
export default function Side() {
  ...
  const FileInfo = () => {
    return (
      ...
      >
        <Dropdown open={open3} onOpenChange={setOpen3}>
          <MenuButton
          ...
          <Menu placement="top-start">
            <Box sx={{ width: 260, ml: 1.8, mr: 1.8, mt: 0.6, mb: 0.6 }}>
              <Stack
                ...
                sx={{ mb: 1 }}
              >
                <Typography level="title-sm">File Information</Typography>
              </Stack>
    ...
    >
      <List sx={{ width: "100%", minWidth: 180, maxWidth: 360 }}>
        ...
        <ErrorBoundary fallback={<FallbackUI />}>
          <Collapse in={open1}>
            <List sx={{ ml: 4.5 }}>
              {objectTypeOverview.length ? (
                objectTypeOverview.map((item, index) => (
                  <Box>
                    <ListItem
                      key={item.name}
                      sx={{
                    ...
                    {index != objectTypeOverview.length - 1 && (
